...
async function ossUpload ({credentials, COMPONENT_PATH, package} = {}) {
  let mainFile
  ;await (async function () {
    const componentFiles = glob.sync('{dist/**/*.*,README.md,icon.png,cover.png,changelog.md}', {cwd: CWD, absolute: false})
...
async function checkComponent ({name, namespace, version}) {
  await request({
    url: FIND_COMPONENT,
...
async function start () {
  ...
  let mainFile = await ossUpload({credentials, COMPONENT_PATH, package})
