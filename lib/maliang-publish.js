...
async function initConfig () {
  let host
  try {
    ENV = await configUtil.getConfig('env')
...
async function getAccessInfo() {
  ...
  util.log('\n')
  try {
    key = await configUtil.getConfig('token')
    key = String(key || '').trim()
  } catch (e) {}  
  if (key) {
    useNewKey = !(await util.confirm(`取得鉴权key【${key}】，是否使用？`))
  }
  if (!key || useNewKey) {
    key = await util.quiz('请输入您的鉴权key(在【码良后台-用户设置-秘钥】页面可获取)')
    try {
      await configUtil.setConfig('token', key)
    } catch (e) {}
  }
  return key
}
...
async function start () {
  ...
  // let CLI_KEY = await configUtil.getConfig('token')
  util.logGreen('\n授权KEY')
  util.log(CLI_KEY)
